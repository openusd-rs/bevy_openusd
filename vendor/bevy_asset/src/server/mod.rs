...
pub(crate) struct AssetServerData {
    source_loads: RwLock<HashMap<AssetPath<'static>, Weak<async_lock::Mutex<()>>>>,
...
impl AssetServer {
    ...
    pub(crate) fn new_with_loaders(
        ...
        Self {
            data: Arc::new(AssetServerData {
                source_loads: RwLock::new(HashMap::default()),
                sources,
                mode,
                meta_check,
                asset_event_sender,
    ...
    async fn load_internal<'a>(
        ...
        let source_lock = {
            ...
            let key = path.without_label().into_owned();
            if let Some(lock) = loads.get(&key).and_then(Weak::upgrade) {
                lock
            } else {
                let lock = Arc::new(async_lock::Mutex::new(()));
