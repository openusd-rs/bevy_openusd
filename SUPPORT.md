...
| Display primvars | Meshes/primitives, point clouds and point prototypes inherit authored nonblocked constant display color/opacity; point clouds also project indexed vertex/varying color/opacity; meshes and mesh point prototypes inherit constant st/st0 UVs; local overrides, owner-local sampled indices, independent clocks and parent-edit invalidation | General shader primvar-reader networks and arbitrary per-texture UV sets remain incomplete; curve display primvars and arbitrary nonconstant shape primvars are not projected |
...
| Material subsets | Ordinary meshes and direct point prototypes, per-subset preview materials, unassigned-face remainder, sampled membership/materials, stable generated children; CPU deformation and ordinary GPU skinning with a shared joint palette | Overlapping/invalid faces fall back to the whole mesh with a diagnostic; GPU point-prototype deformation remains incomplete; splitting duplicates vertex buffers and is not performance-validated |
