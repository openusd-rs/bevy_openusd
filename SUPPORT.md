...
| Authoring | Safe snippets, typed references, immutable UsdSource::with_reference composition with captured dependencies, canonical transaction-backed EditorSession with nested atomic batches; typed schema/component and reusable assembly examples | Source reference composition requires a USDA receiver and new destination prims; each call opens/serializes stages. General reusable typed scene-building API remains incomplete; these APIs are not full BSN equivalence |
