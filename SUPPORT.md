...
| Display primvars | Meshes/primitives, curves, point clouds and point prototypes inherit authored nonblocked constant display color/opacity; curves also project indexed uniform color/opacity, point clouds indexed vertex/varying color/opacity; meshes and mesh point prototypes inherit constant st/st0 UVs; local overrides, owner-local sampled indices and parent-edit invalidation | General shader primvar-reader networks and arbitrary per-texture UV sets remain incomplete; multi-value vertex/varying curve primvars and arbitrary nonconstant shape primvars are not projected |
