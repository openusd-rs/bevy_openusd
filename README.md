...
Try the [self-contained animation showcase](SHOWCASE.md) for composed references,
shared prototypes, skeletal/morph animation and embedded dome lighting.
...
snapshot-only reopening of a relocated package. The viewer can create, edit and
...
**Edit local reference entries** exposes asset path, prim target, time offset and
...
rendering or incomplete uploads. Before reporting success, it also runs
`scripts/check_viewer_log.sh` against the viewer log. Known Rust/Bevy panic and
fatal-runtime markers fail the overall command even if the host still paints a
valid-looking image. Missing/unreadable logs fail closed. Settings record
